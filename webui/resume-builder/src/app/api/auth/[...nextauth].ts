  ...
  return await NextAuth(req, res, {
    providers: [
      AzureADB2CProvider({
        ...
        issuer: `https://${process.env.NEXT_PUBLIC_TENANT_DOMAIN}/${process.env.AZURE_AD_B2C_TENANT_NAME}.onmicrosoft.com/${process.env.AZURE_AD_B2C_PRIMARY_USER_FLOW}/v2.0`,
        authorization: {
          url: `https://${process.env.NEXT_PUBLIC_TENANT_DOMAIN}/${process.env.AZURE_AD_B2C_TENANT_NAME}.onmicrosoft.com/oauth2/v2.0/authorize?p=${process.env.AZURE_AD_B2C_PRIMARY_USER_FLOW}`,
          params: {
            scope: `offline_access openid https://${process.env.NEXT_PUBLIC_TENANT_NAME}.onmicrosoft.com/${AZURE_AD_B2C_API_SCOPE}`,
            p: process.env.AZURE_AD_B2C_PRIMARY_USER_FLOW,
          },
        },
        checks: ['pkce'],
      }),
      ...
      async jwt({
        ...
        if (account !== undefined && account !== null) {
          ...
          token.accessTokenExpires = isANumber(account.expires_on)
            ...
            : undefined;
          token.refreshTokenExpires = isANumber(
    ...
    events: {
      signOut: async () => {
        const cookieStore = await cookies();
        cookieStore.delete('kmp-token');
        cookieStore.delete('kmp-refresh');
      },
    },
  });
}
...
async function refreshAccessToken(token: JWT) {
  try {
    ...
    params.append(
      'scope',
      `openid offline_access https://${process.env.NEXT_PUBLIC_TENANT_NAME}.onmicrosoft.com/kmp-api/kmp.api.all`
    );

    const response = await fetch(
      `https://${process.env.NEXT_PUBLIC_TENANT_DOMAIN}/${process.env.AZURE_AD_B2C_TENANT_NAME}.onmicrosoft.com/${process.env.AZURE_AD_B2C_PRIMARY_USER_FLOW}/oauth2/v2.0/token`,
      {
        method: 'POST',
        headers: {
